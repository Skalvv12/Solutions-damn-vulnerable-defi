...
describe("[Challenge] Side entrance", function () {
	...
	it("Exploit", async function () {
		...
		this.attackContract = await attackContractFactory.deploy(this.pool.address);
		await this.attackContract.attack(ETHER_IN_POOL);
		console.log(
			(await ethers.provider.getBalance(this.attackContract.address)).toString()
		);
		console.log((await ethers.provider.getBalance(this.pool.address)).toString());
